...
''' DEV Notes: Building up the queries

# 1. Get all days that had errors
SELECT path, status, time from log where status like '%404%';

# 2. Get errors by day. Do some aggregating by time (day). \
    Gonna have to manipulate time field to extract day
...
SELECT daily_requests.date, daily_errors.errors::decimal, \
daily_requests.requests::decimal, \
round(((daily_errors.errors::decimal \
/ daily_requests.requests::decimal) * 100), 2) as pct
